...
class LifestylesController extends ShController
{
	public $model2;
	public $_model;
	...
	public function actionGetVariations() {
		if(Yii::app()->request->isAjaxRequest && isset($_POST['lifestyle'])) {
			$lifestyle = Lifestyles::model()->findByPk($_POST['lifestyle']); 
			echo CHtml::hiddenField('product_id', $lifestyle->product_id);

			if($variations = $lifestyle->getVariations()) {
				foreach($variations as $variation) {
					...
					if($variation[0]->specification->input_type == 'textfield') {
						...
						echo CHtml::radioButtonList($field,
								...
								? $variation[0]->id 
								: null,
								LifeStyleVariation::listData($variation, 
									$lifestyle->variationCount > 1 ? true : false
									), array(
	...
	{
		$searchForm=new SearchLifestylesForm();
		
		if(isset($_GET['SearchLifestylesForm']))
			$searchForm->attributes=$_GET['SearchLifestylesForm'];
	...
	public function actionByPetId($id){
		$lifeStyles=Lifestyles::model()->find('pet_id=?',array($id));
		$this->redirect(array('lifestyles/'.$lifestyles->id));
	...
	public function actionView()
	{
		...
		$lifestyle = $this->loadModel();
	 
		$chatForm->product_id=$lifestyle->id;
		$chatForm->product_name=$lifestyle->title;
		$chatForm->pet_id=$lifestyle->id;
		 
		$chatQuestionForm->product_id=$lifestyle->id;
		$chatQuestionForm->product_name=$lifestyle->title;
		$chatQuestionForm->pet_id=$lifestyle->id;
		$feedbacks = new AppComment('search');
		$feedbacks->unsetAttributes();  // actionGetVariations any default values
		$feedbacks->id = $lifestyle->id;
		$feedbacks->table_name ="app_post";
		if(!$lifestyle )
			throw new CHttpException(404);

		$fb_image_path=$this->findProdImgPath($lifestyle);

		$fb_desc="I have recently lost my pet ".$lifestyle->title.", the details are all in this link, If you see ".$lifestyle->title." or have any info, please reach out!!!! PLEASE HELP!!!!";
		//fb metatag 20160321
		...
		// Update View Count
		if($lifestyle)
		$this->render(Shop::module()->lifestyleView,array(
					'lifestyle'=>$lifestyle,
					  'feedback'=>$feedback,
					'feedbacks'=>$feedbacks,
					 'chatForm'=>$chatForm,
	...
	{
		$this->layout = Shop::module()->adminLayout;
		$model=new LifeStyles('search');
		if(isset($_GET['LifeStyles']))
			$model->attributes=$_GET['LifeStyles'];
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=Lifestyles::model()->findbyPk($_GET['id']);
			if(isset($_GET['title']))
				$this->_model=Lifestyles::model()->find('title = :title', array(
							':title' => $_GET['title']));
			if($this->_model===null)
	...
	{
		return $this->renderPartial('_grid', array('lifestyle'=>$data), true);
	}
	...
	private function findProdImgPath($lifestyle){
			if($lifestyle->images) {
