...
class OrderController extends ShController
{
	...
	public function actionCreate(
		...
		if(isset($_POST['PaymentMethod'])){
			if(isset($_POST['PaymentMethod']['id'])){
		...
		if (isset($paymentMethod->id)) {
			if (Shop::getShippingMethodFromPaymentMethod($paymentMethod->id)!=2)
				$skipAddress = false;
			$shipping_method = ShippingMethod::model()->findAll('id = :id', array(':id' => Shop::getShippingMethodFromPaymentMethod($paymentMethod->id)));
		...
		if((!$skipAddress && $customer && $deliveryAddress && $BillingAddress && $customer->customer_id && $deliveryAddress->id && $BillingAddress->id && $paymentMethod && $shipping_method) || 
			($skipAddress && $customer && $customer->customer_id && $paymentMethod && $shipping_method)){
			$customer = Customer::model()->find('user_id = :user_id ', array(
						':user_id' => Yii::app()->user->id));
			if($shipping_option)
	...
	public function actionReview(
		...
		if ($skipAddress) {
			...
			if(isset($_POST['Customer']))
				...
				if(isset($_POST['DeliveryAddress'])) {
					if(Address::isEmpty($_POST['DeliveryAddress'])) {
						Shop::setFlash(Shop::t('Delivery address is not complete! Please fill in all fields to set the Delivery address'));
						echo Yii::trace(CVarDumper::dumpAsString($_POST['DeliveryAddress']),'vardumpx');
					} else {
						//$deliveryAddress = new DeliveryAddress;
						$deliveryAddress->attributes = $_POST['DeliveryAddress'];
						if($deliveryAddress->save()) {
		...
		if(!$shipping_option)
			$shipping_option = Yii::app()->user->getState('shipping_option');
		//echo Yii::trace(CVarDumper::dumpAsString($shipping_method),'vardumpx');
		if((!$skipAddress && $customer && $deliveryAddress && $BillingAddress && $customer->customer_id && $deliveryAddress->id && $BillingAddress->id && $paymentMethod && $paymentMethod->id && $shipping_method) || 
			($skipAddress && $customer && $customer->customer_id && $paymentMethod && $paymentMethod->id && $shipping_method)){
			$customer = Customer::model()->find('user_id = :user_id ', array(
						':user_id' => Yii::app()->user->id));
			echo Yii::trace(CVarDumper::dumpAsString('case1'),'vardumpx');
			echo Yii::trace(CVarDumper::dumpAsString($paymentMethod),'vardumpx');
		...
		echo Yii::trace(CVarDumper::dumpAsString('case2'),'vardumpx');
		echo Yii::trace(CVarDumper::dumpAsString($paymentMethod),'vardumpx');
			$this->render('/order/create', array(
						'customer' => $customer,
						'deliveryAddress' => $deliveryAddress,
	...
	public function actionConfirm() {
		...
		//if(isset($_POST['accept_terms']) && $_POST['accept_terms'] == 1) {
			...
			$skipAddress = true;
			if (Shop::getShippingMethodFromPaymentMethod(Shop::getPaymentMethod())!=2)
				$skipAddress = false;
			...
			if($order->save()) {
				...
				//echo Yii::trace(CVarDumper::dumpAsString(Shop::getShippingMethod()),'vardumpx');
				$position = $this->_add_order_shipping_method($order, Shop::getShippingMethod());
	...
	private function _add_order_shipping_method($order, $shippingMethod)
	{
		$position = new OrderPosition;
		$position->order_id = $order->order_id;
		$position->product_id = 0;
		$position->amount = 1;
		$position->unit_price = ''.$shippingMethod->price;
		$position->total_price = ''.$shippingMethod->price;
		$position->specifications = 'null';
		$position->is_fee = 'Y';
		$position->fee_desc = $shippingMethod->title;
		$position->fee_type = 'shipping';
		$position->ship_country = '';
		$position->store_id = $order->store_id;
		$position->save();
		//echo Yii::trace(CVarDumper::dumpAsString($position),'vardump');
		return $position;
	}
