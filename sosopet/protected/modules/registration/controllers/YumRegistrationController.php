...
class YumRegistrationController extends YumController {
  ...
  public function sendRegistrationEmail($user) {
	...
	*/
	$body = strtr(AppProfile::model()->getMailTemplateRegistrationBody(), array(
      '{username}' => $user->username,
      '{activation_url}' => $activation_url));
	...
	$mail = array(
      ...
      'to' => $user->profile->email,
      'subject' => strtr(AppProfile::model()->getMailTemplateRegistrationSubject(), array(
        '{username}' => $user->username)),
      'body' => $body,
    );
            ...
            if (isset($_POST['YumUserChangePassword'])) {
              ...
              if ($passwordform->validate()) {
				...
				if(Yum::module()->passwordOption=='MD5'){
					$user->activationKey = md5(microtime() . $passwordform->password);
				}else{
        ...
        if ($form->validate()) {
          if($form->user instanceof YumUser) {
            ...
            $mail = array(
              ...
              'subject' => AppProfile::model()->getMailTemplateRecoverSubject(),
              'body' => strtr(
                AppProfile::model()->getMailTemplateRecoverBody(), array(
                  '{recovery_url}' => $recovery_url)),
            );
            $sent = YumMailer::send($mail);
            Yum::setFlash(
            ...
            Yum::log(Yum::t(
			 ...
			 Yum::setFlash(
              'A password has been requested, but no associated user was found in the database.');
		  }
          $this->redirect(Yum::module()->loginUrl);
        }
      }
