...
class YumProfileController extends YumController {
	...
	public function actionUpdate($id = null) {
		...
		$changePassword = new YumUserChangePassword();

		//if(isset($_POST['YumUser']) || isset($_POST['YumProfile'])) {
		if(isset($_POST['YumUser']) || isset($_POST['YumProfile']) || isset($_POST['YumUserChangePassword'])) {
			$user->attributes=@$_POST['YumUser'];
			$profile->attributes = @$_POST['YumProfile'];
			$changePassword->attributes = @$_POST['YumUserChangePassword'];
			...
			$profile->validate();
			$user->validate();
			if ($changePassword->attributes['currentPassword']<>'') {
				$changePassword->validate();
			}

			//if(!$user->hasErrors() && !$profile->hasErrors()) {
			if(!$user->hasErrors() && !$profile->hasErrors() && !$changePassword->hasErrors()) {
				if ($changePassword->attributes['currentPassword']<>'') {
					$user->attributes['password'] = $user->setPassword($changePassword->attributes['password']);
				}
