...
class YumUser extends YumActiveRecord
{
  ...
  public function setPassword($password) {
    if ($password) {
      ...
      if ($this->validate()) {
		if(Yum::module()->passwordOption=='MD5'){
  ...
  public function relations() {
	...
	// Created 20140417
	public function getHybridAuthUsername($hybridAuthProfile, $provider) {
		if ($provider == 'twitter') {
			return $hybridAuthProfile->firstName.'@twitter';
		}else{
			return $hybridAuthProfile->email;
		}
	}
	
	// Created 20140417
	public function registerByHybridAuthProvider($hybridAuthProfile, $provider) {
		Yii::import('profile.models.*');
		...
		$this->username = $this->getHybridAuthUsername($hybridAuthProfile, $provider);
		$this->status = 1;
		$this->createtime = time();
	...
	public function generateActivationKey($activate = false) {
		if($activate) {
			...
			if(Yum::module()->passwordOption=='MD5')
				$this->activationKey = md5(microtime() . $this->password);
			else
	...
	public function attributeLabels()
	{
		return array(
				...
				'verifyPassword' => Yii::t('shop',"Retype password"),
		...
		if (Yum::hasModule('avatar') && $this->profile) {
			...
			if (isset($this->avatar) && $this->avatar)
				...
				$return .= CHtml::image(Yii::app()->getAssetManager()->publish(
							Yii::getPathOfAlias('user.assets.images') . ($thumb
