	...
	public function loginByHybridAuth($provider) {
		...
		try {
			...
			if($success && $success->isUserConnected()) {
				...
				$user = $this->getUser(YumUser::model()->getHybridAuthUsername($hybridAuthProfile, $provider));
				...
				if(!$user) {
					...
					// Updated 20140417
					if(!$user->registerByHybridAuthProvider($hybridAuthProfile, $provider)) {
				...
				if($identity->authenticate(true)) {
					Yum::log(Yum::t('User {username} logged in by hybrid {provider}', array(
									...
									'{provider}' => $provider)));
					//echo Yii::trace(CVarDumper::dumpAsString($hybridAuthProfile),'vardump');
					//throw new CHttpException(403, Yum::t('Permission deniedxxxxx'));
