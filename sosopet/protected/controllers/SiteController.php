...
class SiteController extends Controller
{
	...
	public function accessRules() {
		return array(
				array('allow',
					'actions'=>array('index', 'error', 'contact', 'captcha', 'logout', 'faq', 'testimonial','howitworks', 'aboutus','contactajax','privacypolicy','freetag'),
					'users' => array('*'),
					),
	...
	public function actionContact()
	{
		...
		if(isset($_POST['ContactForm']))
		{
			...
			if($model->validate())
			{
				...
				$body = str_replace('[Enquirer PetName]', $model->petName, $body);
				$body = str_replace('[Enquirer ContactNo]', $model->contactNo, $body);
	...
	public function actionContactajax()
        {
                ...
                if(isset($_POST['ContactajaxForm']))
                {
                        ...
                        {	
                                ...
                                $profile = YumProfile::model()->findByAttributes(array('user_id'=>1,)); //admin email
                                if (isset($profile)){
                                        $this->sendNotification(AppProfile::model()->getEmailUsername(),$profile->email,$subject,$body);echo "Send!";
                                } else{
				echo "Fail";
                                }
			}
                ...
                //$this->render('contactajaxform',array('model'=>$model));
                //$this->render('contactajax');
        }
	
        public function actionFreetag()
        {
                $model=new FreetagForm;
                if(isset($_POST['FreetagForm']))
                {
                        $model->attributes=$_POST['FreetagForm'];
                        if($model->validate())
                        {
                                Yii::import('profile.models.*');
                                //$headers="From: {$model->email}\r\nReply-To: {$model->email}";
                ...
                $this->render('freetag',array('model'=>$model));
